...
| D2 | Does the bounded revision loop re-invoke **S3** with critic findings, or does **G2** emit edits directly? | Re-invoke S3. `TAILORING_METHODOLOGY.md` §4 says "the tailor revises", and it keeps every edit inside the one contract that is already fabrication-proof. This is the assumption both the M8P-4 design and plan are written against. |
...
| D4 | Are `applications/` directories and rendered PDFs git-tracked (methodology §3) or gitignored (public-repo PII constraint)? | Gitignored. The PII constraint is a hard user rule; methodology §3 predates it. Record the deviation in `DECISIONS.md` at the M8P-5 integration commit. |
