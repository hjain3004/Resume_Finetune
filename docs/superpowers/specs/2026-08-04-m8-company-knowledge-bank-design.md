...
Build a versioned, evidence-anchored knowledge bank for 20 approved target companies so
...
Version 0.1.0 covers exactly these 20 canonical company ids and display names:
...
contract. Expanding beyond these 20 is not part of the first implementation or research run.
...
`seed_companies.yaml` fixes the 20-company acceptance set and the expected display name for
...
Research the fixed 20 companies in four batches (Batch 1 covers five companies; Batches 2-4
...
Treat all 20 bundles as untrusted. Validate the complete set, import only if all 20 pass,
generate the human-readable coverage report, run bank tests and the full suite, and commit
the canonical 20-company corpus. No web research occurs in this track.
...
- Twenty dossiers provide useful coverage without attempting an unmaintainable universal
  database.
