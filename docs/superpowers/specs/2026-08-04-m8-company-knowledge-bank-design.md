...
Build a versioned, evidence-anchored knowledge bank for 21 approved target companies so
...
Version 0.1.0 covers exactly these 21 canonical company ids and display names:
...
contract. Expanding beyond these 21 is not part of the first implementation or research run.
...
`seed_companies.yaml` fixes the 21-company acceptance set and the expected display name for
...
Research the fixed 21 companies in four batches (Batch 1 covers six companies; Batches 2-4
...
Treat all 21 bundles as untrusted. Validate the complete set, import only if all 21 pass,
generate the human-readable coverage report, run bank tests and the full suite, and commit
the canonical 21-company corpus. No web research occurs in this track.
...
- Twenty-one dossiers provide useful coverage without attempting an unmaintainable universal
  database.
