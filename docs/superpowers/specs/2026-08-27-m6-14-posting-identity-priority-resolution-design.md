...
**Status:** APPROVED & IMPLEMENTED (2026-08-27)  
...
- `canonical_job_url(raw_url: str) -> str`: Normalizes scheme (HTTP/HTTPS only), lowercase hostname, strips default ports (80/443), strips trailing slashes on non-root paths, strips tracking parameters (`utm_*`, `fbclid`, `gclid`), and deterministically sorts remaining parameters while preserving fragments.
- Validates against embedded userinfo (`user:pass@`) and sensitive query/fragment parameter keys (`token`, `access_token`, `auth`, `authorization`, `api_key`, `apikey`, `secret`, `signature`, `session`, `sessionid`, `jwt`), raising `ManualUrlError`.
...
- `stable_posting_identity(raw_url: str) -> str | None`: Extracts conservative ATS/board identifiers:
  ...
  - `ashby:<org>:<posting_id>` from `jobs.ashbyhq.com`
  - `lever:<org>:<posting_id>` from `jobs.lever.co`
  - `workday:<hostname>:<slug>` from `*.myworkdayjobs.com`
...
2. **CLI Flags & Fail-Closed Validation:**
   ...
   - Requires `--resolve-only`; mutually exclusive with `--discover-only`, `--source`, and `--resolve-limit`.
