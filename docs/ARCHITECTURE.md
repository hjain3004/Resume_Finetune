...
- **Apply-Now tailoring lane** (M8N, 2026-09-01; multi-provider 2026-09-17; provenance repair 2026-09-17): `scripts/tailor_now.py`
  is a file-fed entry to the same S1→S0→S2→S3→G1→G2→RENDER+L7→G3 chain the DB-fed pilot runs
  (`src/tailor/pilot.run_stages`). Input is a JD file plus company, title, base variant, and bound
  provenance sidecar (`.provenance.json`); job ids are negative and content-derived; outputs land
  under gitignored `applications_manual/`. Supports `--provider {claude,gemini,openai}` with
  tool-free, isolated-cwd model execution (`src/tailor/invoke.py`, `src/tailor/providers.py`),
  preserving pure text-in/text-out file contracts. Strict JD provenance boundary: file-fed tailoring
  strictly requires either State A (Database-verified ATS job via `--db`) or State B (User-attested
  official copy via `scripts.tailor_now attest`). All other states fail closed. Manifest schema
  `m8n0.lane_manifest.v2` cryptographically binds the canonical provenance fingerprint, source URL,
  ATS URL, and attestation digest, preventing unverified or aggregator summary relabeling. Design:
