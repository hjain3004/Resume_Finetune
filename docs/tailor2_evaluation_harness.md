...
The lane now follows an artifact-first sanitize-and-deliver policy. Missing evidence bookkeeping, aliases, ordering, and soft section budgets are corrected deterministically; omissions are recorded without a model retry. Per-element unsafe claims are replaced with canonical wording or quarantined, preserving a machine-readable `quarantine_ledger` and the last safe candidate. At most one optional targeted repair call is allowed. A safe candidate is checkpointed before audit, and audit defects downgrade to `NEEDS_HUMAN_REVIEW` when the sanitized artifact remains deliverable. `REJECTED_FATAL` is reserved for corrupt canonical sources, no grounded fallback, irrecoverable unsafe content, failed rendering/source fallback, or impossible artifact provenance. Resume callers can supply the preserved draft and record reused/repeated stages in `recovery_artifacts`.
