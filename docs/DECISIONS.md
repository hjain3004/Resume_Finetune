...
3. **Fail-closed credential rejection in manual inbox:** Embedded userinfo (`user:pass@`) and sensitive query/fragment parameter keys (`token`, `access_token`, `auth`, `authorization`, `api_key`, `apikey`, `secret`, `signature`, `session`, `sessionid`, `jwt`) raise `InboxInputError`, aborting all insertions, leaving `inbox/urls.txt` byte-identical, and sanitizing error messages to prevent secret or URL leakage.
4. **Targeted priority resolution CLI (`--resolve-job-id`):** Provides a fail-closed path to resolve specific `DISCOVERED` jobs without running through the general backlog. Requires `--resolve-only`, forbids discovery flags, `--source`, and `--resolve-limit`, rejects duplicates and non-existent or non-`DISCOVERED` IDs before `db.start_run()`, and restricts pre- and post-resolution eligibility gates strictly to the targeted IDs.
5. **No live network or model calls during development/testing:** Verification executed entirely offline against deterministic unit tests and fixtures.

**Verification.** Full test suite: 1622 passed, 1 deselected in 60s (net +41 tests over M8P-7 baseline). `git diff --check` clean. `data/jobs.db` SHA-256 unchanged: `a9966f4afa4771b61e5b1838c9930e4c64062dc9d85d6fbb49cef17447843ae1`.

**Confirmed:** Zero live model calls, zero live network access, zero unapproved dependencies, zero database corruption. M6.14 is complete offline.
