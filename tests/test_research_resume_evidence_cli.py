...
def test_runner_arguments_are_allowlisted_and_spacing_is_enforced(tmp_path):
    ...
    run_scrape_batch(targets, tmp_path, runner, waits.append, 5, clock=lambda: 0.0)
    assert waits == [2.0]
    assert all(call.count("--proxy") == 1 and call[call.index("--proxy") + 1] == "basic" for call in calls)
    forbidden = {"--profile", "--actions", "--cookie", "--api-key", "--enhanced-proxy", "auto", "stealth"}
...
def test_nonzero_firecrawl_stops_without_retry(tmp_path):
    ...
    def runner(argv):
        calls.append(argv)
        return SimpleNamespace(returncode=7, stdout="", stderr="policy rejection")
    with pytest.raises(ValueError):
...
def test_retrieval_failure_can_use_one_explicit_crawl4ai_fallback(tmp_path):
    def runner(argv):
        return SimpleNamespace(returncode=1, stdout="", stderr="All scraping engines failed to retrieve content from this URL.")
    def fallback(url):
        return {"markdown": "## Synthetic", "html": "<h2>Synthetic</h2>", "links": [], "final_url": url}
    result = run_scrape_batch((ResearchTarget("a", "https://huntr.co/resume-examples/a", "huntr", None),), tmp_path, runner, lambda _: None, 5, fallback, allow_crawl4ai_fallback=True)
...
def test_non_retrieval_failures_cannot_use_fallback(tmp_path, failure_kind):
    ...
    def runner(argv):
        message = {
            ...
            "provider_refusal": "provider refused",
            "policy_rejection": "policy rejection",
            "exhausted_budget": "budget exhausted",
        }[failure_kind]
        return SimpleNamespace(returncode=1, stdout="", stderr=message)
    with pytest.raises(ValueError):
        run_scrape_batch((ResearchTarget("a", "https://huntr.co/resume-examples/a", "huntr", None),), tmp_path, runner, lambda _: None, 5, lambda url: fallback_calls.append(url), allow_crawl4ai_fallback=True)
...
def test_hash_matching_rerun_skips_and_unbound_output_conflicts(tmp_path):
    ...
    run_scrape_batch((target,), tmp_path, runner, lambda _: None, 5)
