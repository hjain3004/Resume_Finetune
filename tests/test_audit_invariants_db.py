...
def test_i6a_fail_when_a_scored_row_has_explicit_no_sponsorship():
    conn = _conn()
    conn.execute(
        """
        INSERT INTO jobs (dedup_key, company, title, location, url, source, discovered_at, status, jd_text)
        VALUES ('k1', 'Acme', 'Software Engineer', 'New York, NY', 'https://acme.example/1', 'tracker_vansh', '2026-07-01T00:00:00+00:00', 'SCORED', 'Starts in 2027. Unable to sponsor visas.')
        """
    )
    conn.commit()
    finding = check_i6a(conn, _AUDIT_CFG, _FILTERS_CFG, load_eligibility_config(), {}, None)
    assert finding.status == "FAIL"
    assert finding.evidence[0]["reason"] == "eligibility:work_authorization"


def test_i6a_fail_when_active_row_has_explicit_canada_location():
    conn = _conn()
    conn.execute(
        """
        INSERT INTO jobs (dedup_key, company, title, location, url, source, discovered_at, status, jd_text)
        VALUES ('k1', 'Acme', 'Software Engineer', 'Toronto, Canada', 'https://acme.example/1', 'tracker_vansh', '2026-07-01T00:00:00+00:00', 'SCORED', 'Starts in 2027.')
        """
    )
    conn.commit()
    finding = check_i6a(conn, _AUDIT_CFG, _FILTERS_CFG, load_eligibility_config(), {}, None)
    assert finding.status == "FAIL"
    assert finding.evidence[0]["reason"] == "eligibility:country"
...
def test_i6a_pass_when_titles_all_pass_prefilter():
    ...
    conn.execute(
        """
        INSERT INTO jobs (dedup_key, company, title, location, url, source, discovered_at, status, jd_text)
        VALUES ('k1', 'Acme', 'Software Engineer', 'Remote', 'https://acme.example/1', 'tracker_vansh', '2026-07-01T00:00:00+00:00', 'SCORED', 'Starts in 2027.')
        """
    )
    conn.commit()
    finding = check_i6a(conn, _AUDIT_CFG, _FILTERS_CFG, load_eligibility_config(), {}, None)
...
def test_i6b_warn_when_run_filters_over_90_percent():
    ...
    finding = check_i6b(conn, _AUDIT_CFG, _FILTERS_CFG, load_eligibility_config(), {}, None)
...
def test_i8_fail_on_discovered_row_at_resolve_limit():
    ...
    conn.commit()
    finding = check_i8(conn, _AUDIT_CFG, _FILTERS_CFG, load_eligibility_config(), {}, None)
    assert finding.status == "FAIL"
...
def test_i9_warn_first_time_flags_stale_active_row():
    ...
    finding = check_i9(conn, audit_cfg, _FILTERS_CFG, load_eligibility_config(), {}, None)
...
def test_i10_fail_on_orphaned_run_sources_row():
    ...
    conn.commit()
    finding = check_i10(conn, _AUDIT_CFG, _FILTERS_CFG, load_eligibility_config(), {}, None)
    assert finding.status == "FAIL"
...
def test_i7_check_is_skip_in_automatic_audit():
    conn = _conn()
    finding = check_i7(conn, _AUDIT_CFG, _FILTERS_CFG, load_eligibility_config(), {}, None)
