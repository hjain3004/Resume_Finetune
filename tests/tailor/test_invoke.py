...
def test_default_command_shape_has_tools_disabled_and_no_session_persistence():
    assert DEFAULT_S1_CLAUDE_CMD == ("claude", "-p", "--tools", "", "--no-session-persistence", "--")
...
def test_invoke_passes_prompt_as_final_positional_argv_element():
    with patch.object(
        ...
        invoke_s1_model("PROMPT TEXT", claude_cmd=("claude", "-p", "--tools", "", "--no-session-persistence", "--"))

    args, kwargs = mock_run.call_args
    cmd = args[0]
    assert cmd[-1] == "PROMPT TEXT"
    assert cmd[:-1] == ["claude", "-p", "--tools", "", "--no-session-persistence", "--"]
    # No shell invocation.
...
def test_invoke_success_returns_raw_stdout_and_model():
    with patch.object(
        ...
        result = invoke_s1_model("prompt", claude_cmd=("claude", "-p", "--tools", "", "--no-session-persistence", "--"))
